...
test("public link falls back from an invalid requested locale and hides it from language switching", async (t) => {
  process.env.NEXT_PUBLIC_SUPABASE_URL ||= "https://stub.supabase.local";
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ||= "stub-anon-key";
  process.env.SUPABASE_SERVICE_ROLE_KEY ||= "stub-service-role-key";
  ...
  const linkRow = {
    ...
    default_locale: "en", available_locales: ["en", "pl"], is_active: true, status: "active",
  ...
  const snapshotRow = {
    ...
    published_locales: ["en", "pl"], available_locales: ["en", "pl"], selection: null,
  ...
  assert.deepEqual(publicRoute.published.languages.map(({ code }) => code), ["en"]);
  assert.deepEqual(publicRoute.availableLocales, ["en"]);
  ...
  assert.deepEqual(publicExport.availableLocales, ["en"]);
