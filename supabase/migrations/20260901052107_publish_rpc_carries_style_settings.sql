-- Phase 2, step B: freeze the document style into the published snapshot.
--
-- Republished verbatim from 20260604000000_reactivate_revoked_public_link_on_publish.sql
-- with exactly two additions: the snapshot locale insert now carries
-- style_settings, copied from the source document. Everything else is
-- byte-identical, so this migration cannot silently change publishing.
--
-- The style must be frozen here rather than read live from resume_documents:
-- a published CV is an immutable copy, and reading style at render time would
-- let a later edit in the editor restyle a CV somebody had already shared.
--
-- coalesce() guards documents written before 20260901000000 added the column.
