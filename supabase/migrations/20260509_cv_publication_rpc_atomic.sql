-- ADR 0001 PR4: transactional publish/unpublish RPC for Saved Version -> Published CV -> Public Link.
...
begin
  ...
  if exists (
    ...
    where not exists (
      ...
      where rpv.preset_id = preset_row.id
        ...
        and lower(rpv.locale) = locale_value.code
  ...
  if inserted_locales_count <> cardinality(normalized_selected_locales) then
    raise exception 'Snapshot locale row count mismatch.';
  end if;
...
create or replace function public.unpublish_resume_saved_version(
  input_preset_id uuid
)
returns boolean
language plpgsql
...
grant execute on function public.unpublish_resume_saved_version(uuid) to authenticated;
