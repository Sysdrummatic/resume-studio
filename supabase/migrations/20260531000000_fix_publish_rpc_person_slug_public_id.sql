-- Fix: publish_resume_saved_version never set person_slug or public_id on resume_public_links.
-- Both were null, so canonical_public_path could not be derived on the client.
-- This migration re-creates the function with person_slug (from profiles) and
-- public_id (generated or preserved from a prior revoked link) populated on every publish.
...
begin
  ...
  -- Resolve person_slug from profile
  select person_slug
  ...
  -- Verification: check all selected locales have an owned document
  if exists (
    select 1
  ...
  -- Create locale snapshots (fallback to base selection if variant missing)
  ...
  if active_link.id is not null then
    -- Preserve existing public_id or generate a new one
    ...
    -- Reuse public_id from a prior revoked link for URL stability, or generate fresh
